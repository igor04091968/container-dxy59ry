...
func (s *Server) Start() (err error) {
	...
	if portStr != "" {
		...
		if err != nil {
			logger.Warningf("Invalid PORT environment variable: %s. Falling back to settings.", portStr)
			// Fallback to settings if PORT is not a valid integer
			port, err = s.settingService.GetPort()
			if err != nil {
				return err
			}
		} else {
			logger.Infof("Using PORT environment variable: %d", port)
		}
	} else {
		// Fallback to settings if PORT is not set
		port, err = s.settingService.GetPort()
		if err != nil {
			return err
		}
